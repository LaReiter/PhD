...
## Figures and tables *not* reproduced by this folder

The following figures and tables appear in the paper or supplement but are not produced by any R script transferred here. They are static assets in `../graphics/` (likely hand-made or generated by scripts that live elsewhere):

- Main Fig. `fig:intro` (Greenland map, tusk photos), `fig:signals` (element signals for tusk 956), `fig:workflow` (analytical flowchart)
- Supp `fig:bp` (change-point likelihood + fit), `fig:EW_wf` (workflow diagram), `fig:hist`, `fig:hist_3reg` (permutation histograms), `fig:cemden` (cementum–dentine ratios), `fig:el_vs_reg_vs_tusk` (per-tusk trend lines), Supp Fig. S1 (calibration curve)
- Supp Table S2 (3-region permutation test using the F-statistic): no script in this folder implements the 3-region extension; the values shown in the supplement are hand-entered from an earlier analysis.
